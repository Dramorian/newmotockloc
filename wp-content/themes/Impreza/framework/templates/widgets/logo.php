...
if ( us_get_option( 'logo_type', 'text' ) == 'text' ) {
	...
	foreach ( $images_config as $logo_type => $option_name ) {
		if ( ! ( $logo_url = us_get_option( $option_name ) ) ) {
			continue;
		}
		if ( ! empty( $logo_url ) AND $logo_type != 'default' ) {
			$class_name = ' with_' . $logo_type;
		}
		$img = usof_get_image_src( $logo_url );
		if ( $img ) {
			$images_html .= '<img class="for_' . $logo_type . '" src="' . esc_url( $img[0] ) . '" ';
			$images_html .= 'width="' . $img[1] . '" height="' . $img[2] . '" alt="' . esc_attr( $logo_alt ) . '" />';
		}
	}
}
