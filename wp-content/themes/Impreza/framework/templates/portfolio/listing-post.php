...
$placeholder_url = $us_template_directory_uri . '/framework/img/placeholder/us-placeholder-square.png';
...
<div class="w-portfolio-item<?php echo $classes ?>" data-id="<?php the_ID() ?>" data-categories="<?php echo implode( ',', $categories_slugs ) ?>">
	<a class="w-portfolio-item-anchor" href="<?php echo $link ?>"<?php echo $anchor_atts . $anchor_inner_css ?>>
		...
		<?php
		$image2 = rwmb_meta( 'us_tile_additional_image' );
		if ( $image2 != '' AND ( $image2 = wp_get_attachment_image_src( $image2, 'tnail-masonry' ) ) ) {
			echo '<div class="w-portfolio-item-image second" style="background-image: url(' . $image2[0] . ')">';
